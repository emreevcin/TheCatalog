...
public class MainController extends TreeCell<String> implements Initializable {
    ...
    void createType() {
        if (typeTF.getText().length() != 0) {
            ...
            TableColumn header = new TableColumn(typeTF.getText());
            typeTable.getColumns().add(header);
            if(keyTF.getText().length() != 0){
                attributes.add(keyTF.getText());
                TableColumn att = new TableColumn(keyTF.getText());
                typeTable.getColumns().add(att);
